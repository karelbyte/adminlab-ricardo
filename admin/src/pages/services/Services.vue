<template>
  <q-page class="q-pa-md">
    ...
    <div id="print" hidden>
      <span style="font-size: 8px; margin-left: 25px">{{
        toPrint.name.substr(0, 17)
      }}</span
      ><br />
      <span
        style="
    ...
    <div v-if="views.list">
      <div class="row">
        <div class="col-lg-8 col-xs-12">
          ...
          <q-btn color="warning" label="INFORME DIARIO" @click="openInfo()" />
        </div>
                    ...
                    <q-btn color="primary" flat icon="fa fa-ellipsis-h">
                      ...
                      >
                        <q-list style="min-width: 100px">
                          ...
                          <q-item clickable>
                            ...
                            <q-menu anchor="top right" self="top left">
                              <q-list style="min-width: 100px">
                                <div
                                  v-for="des in props.row.analysis"
                                  :key="des.id"
                                >
                                  <q-item
                                    clickable
                        ...
                        <q-item
                          ...
                          clickable
                          v-close-popup
                          @click="formChangeStatusService(props.row)"
                        >
                          <q-item-section avatar>
    ...
    <div v-if="views.news">
      <q-card style="width: 100%">
        ...
        <div class="row">
          ...
          <div
            v-if="client !== null && item.barcode !== ''"
            style="position: absolute; left: calc(70vw - 80px)"
          >
            <span style="font-size: 10px; margin-left: 10px">{{
              client.names.substr(0, 16)
            }}</span
            ><br />
                ...
                <q-item v-bind="scope.itemProps" v-on="scope.itemEvents">
                  <q-item-section>
                    <q-item-label caption>{{
                      scope.opt.names + " (" + scope.opt.symbol + ")"
                    }}</q-item-label>
                  </q-item-section>
                </q-item>
              </template>
    ...
    <q-dialog
      v-model="showEditResult"
      persistent
      transition-show="scale"
      transition-hide="scale"
    >
      <q-card style="width: 900px; max-width: 80vw">
        <q-card-section class="bg-purple-5"> Resultados </q-card-section>
        <q-separator />
        <q-card-section>
          <tinymce
            id="d1"
            v-model="dataResult.content"
            :toolbar1="tool"
        ...
        <q-card-actions align="right">
          <q-btn
            v-if="statusEditResult === 1"
            dense
            label="Guardar"
            color="positive"
            v-close-popup
            @click="saveContentResult"
          />
          <q-btn dense flat label="Cerrar" v-close-popup />
        </q-card-actions>
      </q-card>
    ...
    <!-- AÑADIR CLIENTES -->
    <ClientAdd
      :showFormClient="showFormClient"
      :name="clientName"
      act="post"
      @update="getClientList"
      @close="closeFormClient"
    ></ClientAdd>
    <!-- AÑADIR Doctor -->
    <DoctorAdd
      :showFormDoctor="showFormDoctor"
      act="post"
      @update="getDoctorList"
      @close="closeFormDoctor"
    ></DoctorAdd>
    <!-- AÑADIR LOCACION -->
    <LocationAdd
      :showFormLocation="showFormLocation"
      act="post"
      @update="getLocationList"
      @close="closeFormLocation"
    ></LocationAdd>
    <!--CAMBIAR ESTADO SERVICIO -->
    <form-change-service-status
      :show="showFormChangeStatusService"
      :item="item"
    ...
    >
      <q-card style="width: 500px; max-width: 80vw">
        <q-card-section> Impresion de codigos de barrar </q-card-section>
        <q-separator />
        <q-card-section>
          <q-input
            dense
            ...
            autogrow
            v-model="qrCant"
            label="Cantidad de codigos de barra"
          />
        </q-card-section>
        <q-separator />
        <q-card-actions align="right">
          <q-btn dense flat label="Imprimir" @click="runPrintQr()" />
          <q-btn dense flat label="Cerrar" @click="showPrintQr = false" />
        </q-card-actions>
      </q-card>
    </q-dialog>
    ...
    >
      <q-card style="width: 400px; max-width: 80vw">
        <q-card-section class="bg-blue-5 text-center"> Enviando email... </q-card-section>
        <q-separator />
