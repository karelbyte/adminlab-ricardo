...
export default {
  ...
  methods: {
    ...
    sendEmail (item) {
      this.showEmail = true
      this.$axios.post(ApiUrl + '/services/mail', { id: item.id }).then(res => {
    ...
    getCodeBar () {
      if (this.location !== null && this.client !== null && this.act === 'post') {
    ...
    runPrintQr () {
      let data = {
        qr: this.item.barcode,
        cant: this.qrCant,
        name: this.item.client.toUpperCase()
      }
      this.$axios.post(ApiUrl + '/printrq', data).then(() => {
