...
class ClientController extends Controller
{
    ...
    {
        Client::query()->where('id', $request->id)->update($request->except('services', 'services_count', 'created_at', 'updated_at'));
    ...
    public function report($id) {
        ...
        $data = Client::with(['services' => function ($q) {
            $q->with('analysis');
        }])->where('id', $id)->first();


        $view = view('info_client_history', ['data' => $data])->render();
